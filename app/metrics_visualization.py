...
# Visualize tab with robust parsing and LLM fallback


def visualize_tab():
    st.subheader("\U0001F4CA Metrics Visualization")

    uploaded_file = st.file_uploader("Upload log file for visualization", type=[
        "txt", "csv", "json", "log", "pdf", "docx", "xls", "xlsx"])
        ...
        try:
            ...
            try:
                ...
                content = uploaded_file.read().decode(
                    "utf-8") if file_type not in ["pdf", "docx"] else ""
                df = fallback_llm_parse(content)
                if df is not None and not df.empty:
                    st.session_state['analyzed_df'] = df.copy()
                else:
    ...
    if df is None or df.empty:
        st.info("Upload a log file to visualize.")
        return
    ...
    threshold = st.slider(f"Set threshold for {selected_kpi}", float(filtered_df[selected_kpi].min(
    )), float(filtered_df[selected_kpi].max()), float(filtered_df[selected_kpi].mean()))
    ...
    if 'Meter_ID' in filtered_df.columns:
        st.markdown("### Fraud Distribution in Texas (Mock Data)")
        ...
        fig_map = px.scatter_mapbox(merged_geo, lat='lat', lon='lon', color=selected_kpi, size=selected_kpi, hover_name='Meter_ID',
                                    zoom=5, mapbox_style='carto-positron', title="Fraud Distribution Map in Texas (Mock Coordinates)")
        ...
        if not anomalies.empty:
            ...
            try:
                ...
                if response.status_code == 201:
                    st.success(
                        f"✅ Ticket #{ticket_id} created successfully!\n\nYour fraud report has been received and logged.")
                    st.json(ticket)
